...
while (!shouldExit)
{
    ...
    Move(playerSpeed: movementSpeed);
    if (TerminalResized())
    {
        ResetGame();
...
void Move(bool otherButtonIsExit = false, int playerSpeed = 1)
{
    int lastX = playerX;
    int lastY = playerY;
...
bool PlayerEatFood()
{
    if ( (playerX >= foodX && playerX < (foodX + foods.Length) ) && playerY == foodY)
    {
        return true;
    }
    return false;
