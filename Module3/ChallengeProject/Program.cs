...
do
{
    ...
    switch (menuSelection)
    {
        ...
        case "3":
            ...
            // Count numbers of the animal we have
            for (int i = 0; i < maxPets; i++)
            {
                if (ourAnimals[i, 0] != "ID #: ")
                {
            ...
            for (int i = 0; i < petCount; i++)
            {
                ...
                while (animalAge == "Age: " || animalAge == "Age: ?")
                {
                    // Remove the ? symbol
                    int indexOfQuestionMark = animalAge.IndexOf("?");
                    if (indexOfQuestionMark != -1)
                        animalAge = animalAge.Remove(indexOfQuestionMark);

                    Console.WriteLine($"Please enter an age for {ourAnimals[i, 0]}");
                    readResult = Console.ReadLine();
                    if (readResult != null)
                    {
                        if (int.TryParse(readResult.Trim(), out petAge))
                        {
                            animalAge = readResult;
                        }
                    }
                ...
                while (animalPhysicalDescription == "Physical description: " || animalPhysicalDescription == "Physical description: tbd")
                {
                    // Remove the tbd text
                    int indexOfQuestionMark = animalPhysicalDescription.IndexOf("tbd");
                    if (indexOfQuestionMark != -1)
                        animalPhysicalDescription = animalPhysicalDescription.Remove(indexOfQuestionMark);

                    Console.WriteLine($"Please enter a physical description for {ourAnimals[i, 0]}");
                    readResult = Console.ReadLine();
                    if (readResult != null)
                    {
                        if (readResult.Trim().Length > 8)
                        {
                        ...
                        {
                            Console.WriteLine($"Try to be more descriptive (min 8 characters)");
                        }
                    }
                }
        ...
        case "4":
            ...
            for (int i = 0; i < petCount; i++)
            {
                ...
                int indexOfColon = animalNickname.IndexOf(":");
                animalNickname = animalNickname.Substring(indexOfColon + 1).Trim();

                while (animalNickname == "" || animalNickname == "tbd")
                {
                    Console.WriteLine($"Please enter an nickname for {ourAnimals[i, 0]}");
                    readResult = Console.ReadLine();
                    if (readResult != null)
                    {
                        if (readResult.Trim().Length > 2)
                        {
                ...
                // Remove the prefix text
                indexOfColon = animalPersonalityDescription.IndexOf(":");
                animalPersonalityDescription = animalPersonalityDescription.Substring(indexOfColon + 1).Trim();
                ...
                // store the pet information in the ourAnimals array (zero based)
                ourAnimals[i, 3] = "Age: " + animalNickname;
